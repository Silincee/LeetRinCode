...
public class BaZiFuChuanZhuanHuanChengZhengShuLcof{
	 public static void main(String[] args) {
		 ...
		 System.out.println(solution.strToInt(" -13s2"));
	 }
//力扣代码
	//leetcode submit region begin(Prohibit modification and deletion)
class Solution {
    public int strToInt(String str) {
		char[] c = str.trim().toCharArray(); // 首部空格： 删除之即可；
		if(c.length == 0) return 0; // 字符串为空
		int res = 0, bndry = Integer.MAX_VALUE / 10; // 数字拼接边界 bndry = 2147483647//10
		int i = 1, sign = 1;
		// 判断符号
		if(c[0] == '-') sign = -1;
		else if(c[0] != '+') i = 0;
		// 若从左向右遍历数字，设当前位字符为 c ，当前位数字为 x ，数字结果为 res ，则数字拼接公式为
		//  res=10×res+x
		for(int j = i; j < c.length; j++) {
			if(c[j] < '0' || c[j] > '9') break; // 遇到首个非数字的字符时，应立即返回。
			// 两种情况越界：
			// 情况一：执行拼接10×res≥2147483650越界(res>bndary)
			// 情况二：拼接后是2147483648或2147483649越界(res=bndary,x>7)
